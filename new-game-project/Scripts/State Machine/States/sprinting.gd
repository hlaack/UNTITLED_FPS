...
func physics_update(delta: float) -> void:
	...
	elif not Input.is_action_pressed("sprint") and not input_dir.is_zero_approx():
...
func ground_move_sprinting(delta) -> void:
	var cur_speed_in_wish_dir = player.velocity.dot(player.wish_dir)
	var add_speed_until_cap = player.sprint_speed - cur_speed_in_wish_dir
	if add_speed_until_cap > 0:
		var accel_speed = player.ground_accel * delta * player.sprint_speed
